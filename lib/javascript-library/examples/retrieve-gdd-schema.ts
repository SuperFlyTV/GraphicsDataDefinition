...
export async function retrieveGDDSchema(htmlFilePath: string): Promise<GDDSchema> {
	...
	if (typeof htmlContent !== 'string') throw new Error(`Bad input data, must be a string (was "${typeof htmlContent}")`)
...
function extractGDDJSONFromHTMLFile(htmlString: string) {
	...
	if (!match)
		throw new Error(`Did not find any <script> tag, did you forget to add it?\n${exampleSchemaTagWithContent}`)
