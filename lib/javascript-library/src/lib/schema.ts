...
export async function retrieveGDDSchema(
  htmlFilePath: string
): Promise<GDDSchema> {
  ...
  let gddSchema: GDDSchema;

  if (typeof htmlContent !== "string")
    throw new Error(
      `Bad input data, must be a string (was "${typeof htmlContent}")`
    );

  // Look for <script name="graphics-data-definition">:
  const match = htmlContent.match(
    /<script([\S\s]*)name=["']graphics-data-definition['"]/
  );
  if (!match)
    throw new Error(
      `Did not find any <script name="graphics-data-definition"> tag, did you forget to add it?\n${exampleSchemaTag}`
    );
...
function extractGDDJSONPath(htmlString: string) {
  const match = htmlString.match(
    /<script([\S\s]*?name=["']graphics-data-definition['"][\S\s]*?)>/
  );
  ...
  if (!tagContent)
    throw new Error(
      `<script> tag is empty, it should be on this form:\n${exampleSchemaTag}`
    );

  const srcMatch =
    tagContent.match(/src="([^"]*)"/) || tagContent.match(/src='([^']*)'/);
...
const exampleSchemaTag =
  ' <script name="graphics-data-definition" type="application/json+gdd" src="path-to-json-file"></script>';
...
function extractGDDJSONFromHTMLFile(htmlString: string) {
  const match =
    ...
    htmlString.match(
      /<script [\S\s]*?name=["']graphics-data-definition['"][\S\s]*?>([\s\S]*?)<\/script>/
    );
  if (!match)
    throw new Error(
      `Did not find any <script> tag, did you forget to add it?\n${exampleSchemaTagWithContent}`
    );
